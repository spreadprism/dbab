...
describe("adapter", function()
	...
	describe("build_cmd", function()
		...
		describe("postgres", function()
			...
			it("asks psql for a null sentinel", function()
				local _, args = adapter.build_cmd("postgres://localhost/mydb")
				assert.is_true(vim.tbl_contains(args, "--pset=null=" .. adapter.NULL_SENTINEL))
			end)
		end)
		...
		describe("sqlite", function()
			it("returns sqlite3 with database path", function()
				...
				assert.are.equal("/tmp/test.db", args[#args])
			end)

			it("asks sqlite3 for a null sentinel", function()
				local _, args = adapter.build_cmd("sqlite:///tmp/test.db")
				assert.are.equal("-nullvalue", args[1])
				assert.are.equal(adapter.NULL_SENTINEL, args[2])
			end)
		end)
