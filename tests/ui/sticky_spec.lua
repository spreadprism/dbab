...
describe("sticky", function()
	...
	describe("set_header", function()
		it("stores the header line", function()
			sticky.set_header(" id | name ")
			assert.are.equal(" id | name ", sticky.header)
		end)
		...
		it("hides the float when set to nil", function()
			...
			assert.is_nil(sticky.header)
			assert.is_nil(sticky.win)
		end)
	end)
	...
	describe("follow", function()
		...
		it("opens a float once scrolled past the header", function()
			...
			assert.is_not_nil(sticky.win)
			assert.is_true(vim.api.nvim_win_is_valid(sticky.win))

			local lines = vim.api.nvim_buf_get_lines(sticky.buf, 0, -1, false)
			assert.are.equal(" id | name ", lines[1])

			local cfg = vim.api.nvim_win_get_config(sticky.win)
		...
		it("reuses the same window instead of reopening it", function()
			...
			sticky.follow(win)
			local first = sticky.win
			sticky.follow(win)

			assert.are.equal(first, sticky.win)
		end)
	...
	describe("skip_header", function()
		...
		it("clamps the column to the line it lands on", function()
			...
			sticky.set_header(" a_very_long_header ")
